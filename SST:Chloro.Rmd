...
url <- paste0("https://thredds.jpl.nasa.gov/thredds/ncss/OceanTemperature/MUR-JPL-L4-GLOB-v4.1.nc?var=analysed_sst&north=34.75&west=-121.4&east=-118.4&south=33.55&disableLLSubset=on&disableProjSubset=on&horizStride=1&time_start=", past_date, "&time_end=", date, "&timeStride=1&addLatLon=true")
...
# 3 day chl composite from aqua modis - might want to choose 8 day for clarity. 
chl_date <- as.Date(Sys.time()) -3
...
url <- paste0("https://coastwatch.pfeg.noaa.gov/erddap/griddap/erdMWchla3day_LonPM180.nc?chlorophyll%5B(", chl_date,")%5D%5B(0.0)%5D%5B(33.55):(34.75)%5D%5B(-121.4):(-118.4)%5D&.draw=surface&.vars=longitude%7Clatitude%7Cchlorophyll&.colorBar=%7C%7C%7C%7C%7C&.bgColor=0xffccccff")

download.file(url, here("data", "chl.nc"))

chl_ras <- here("data","chl.nc")
...
chl_pal <- colorspace::sequential_hcl(25,
...
ras_chl <- raster(chl_ras, col = chl_pal)
...
rm(ras_chl, new_ras_chl, cropped_chl, merged_shapes_mask)
gc()
```
